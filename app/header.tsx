...
export default function Header() {
    ...
    return <header className={"fixed top-0 left-0 mt-5 w-full flex justify-center z-40"}
    >
        <div className={"backdrop-blur-sm bg-transparent tracking-tight text-[0.925rem] flex gap-12 max-md:gap-8 justify-between border border-neutral-800 " +
            "text-neutral-200 py-3 px-5 rounded-xl items-center"}>

        <Link href={"/"} className={"flex gap-2 font-medium text-white justify-center items-center"}>
            <EggFriedIcon size={20} strokeWidth={2}/>
            <div className={"max-md:hidden"}>Food Print</div>
        </Link>
        <Link href={"/"}
        className={`${pathname === "/" ? "text-white font-medium" : ""}`}
        >Dashboard</Link>
        <Link className={"max-md:hidden"} href={"/journal"}>Journal</Link>
